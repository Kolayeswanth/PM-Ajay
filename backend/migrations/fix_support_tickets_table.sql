-- Fix existing support_tickets table by adding missing columns
-- This migration addresses the issue where the table exists but lacks certain columns

-- Drop existing table and recreate with all required columns
DROP TABLE IF EXISTS support_tickets CASCADE;

-- Create Support Tickets table with all required columns
...
COMMENT ON COLUMN support_tickets.user_id IS 'ID of the user who created the ticket';
COMMENT ON COLUMN support_tickets.user_role IS 'Role of the user (contractor, admin, etc.)';
COMMENT ON COLUMN support_tickets.user_name IS 'Name of the user';
COMMENT ON COLUMN support_tickets.user_email IS 'Email of the user';
