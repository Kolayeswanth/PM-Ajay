...
const DepartmentAllProjects = ({ agencyName }) => {
    ...
    // Fetch all projects for this implementing agency from backend
    useEffect(() => {
        if (agencyName) {
            fetchAgencyProjects();
        }
    ...
    const fetchAgencyProjects = async () => {
        try {
            ...
            if (result.success) {
                // For implementing agencies, show all assigned projects
                // This can be filtered by agency if needed
    ...
    const handleExportData = () => {
        ...
        link.setAttribute('download', `Agency_Projects_${new Date().toISOString().split('T')[0]}.csv`);
                    ...
                    <tbody>
                        {filteredProjects.length > 0 ? (
                            filteredProjects.map(project => (
                                <tr key={project.id}>
                                    <td>
                                        <div style={{ fontWeight: 'bold', fontSize: '14px' }}>{project.state_name}</div>
...
export default DepartmentAllProjects;
