...
const StateAllProjects = ({ stateName }) => {
    ...
    // Fetch all districts for this state from backend
    useEffect(() => {
        if (stateName) {
            fetchAllDistricts();
        }
    ...
    const fetchAllDistricts = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/rest/v1/districts?state_name=eq.${stateName}&select=name&order=name.asc`, {
                headers: {
            ...
            if (data) {
                setAllDistricts(data.map(d => d.name));
            }
        } catch (error) {
    ...
    // Fetch all projects for this state from backend
    useEffect(() => {
        if (stateName) {
            fetchStateProjects();
        }
    ...
    const fetchStateProjects = async () => {
        try {
            ...
            if (result.success) {
                // Filter to only show projects for this state
                const stateProjects = result.data.filter(p => p.state_name === stateName);
                setProjects(stateProjects);
                setFilteredProjects(stateProjects);
            } else {
    ...
    useEffect(() => {
        ...
        if (selectedDistrict !== 'all') {
            filtered = filtered.filter(p => p.district_name === selectedDistrict);
        }
        ...
        // Extract unique districts from filtered projects for chat recipients
        const uniqueDistricts = [...new Set(filtered.map(p => p.district_name))].filter(Boolean);
        setChatRecipients(uniqueDistricts);
    }, [selectedDistrict, selectedComponent, selectedStatus, projects]);
    ...
    const handleExportData = () => {
        ...
        const headers = ['District', 'Project Name', 'Component', 'Cost (Lakhs)', 'Status', 'Date'];
        const csvContent = [
            headers.join(','),
        ...
        link.setAttribute('download', `${stateName}_Projects_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div className="dashboard-panel" style={{
            ...
            <ChatInterface
                ...
                onSendMessage={(message, recipients) => {
                    ...
                    showToast(`Message sent to ${recipients.length} district${recipients.length !== 1 ? 's' : ''}!`, 'success');
                }}
            />
        </div>
    );
...
export default StateAllProjects;
